...
Compose injects `docker.env` into the bot container, and the application reads its environment before the optional local `config.env` file, so private deployment values take precedence. `BYPASS_PROXY_POOL` is the shared pool used by social-media downloads, provider/ad-link scrapers, ToonWorld, and the external solver calls; the social downloader defaults to proxy-only mode and therefore requires a configured pool. ToonWorld tries the direct request and then each configured proxy before asking `SOLVER_API` for challenge cookies/HTML. JAVHD solver requests try the same pool and then direct access. Never place authenticated proxy URLs in the Dockerfile, Compose file, or public repository. Configure the pool once in the deployment platform's private environment/secret settings.
